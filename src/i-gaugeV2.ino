...
void setup() {
  ...
  ads.begin();      //ADS1115
  //define supply power
  Serial.println(F("CEK VOLTAGE"));
  voltase = float(ads.readADC_SingleEnded(2)) * 0.1875 / 1000.0000 * 5.325443787;
  Serial.println(voltase);
  if (voltase >= 8.00) {
    digitalWrite(GPower, HIGH); //PLN
  }
  if (voltase < 8.00) {
    digitalWrite(BPower, HIGH); //BATTERY
  }
  ...
  if (! rtc.begin() || ! rtc.isrunning()) {
    ...
    while (1) {
      digitalWrite(RState, HIGH);  delay(500); //LED RED
      digitalWrite(RState, LOW);   delay(500);
    }
  }
...
void ambil() {
  ...
  if (i == 0) {
    file.print(F("TimeStamp (YYYY-MM-DD HH:MM:SS) | Date From Receiver (YYYY-MM-DD HH:MM:SS) | "));
  ...
  //FORMAT DATA = TimeStamp (YYYY-MM-DD HH:MM:SS) | Date From Receiver (YYYY-MM-DD HH:MM:SS) |
  //        Longitude (DD.DDDDDD°) | Latitude (DD.DDDDDD°) | Pressure (BAR)| Temperature (°C) |
  //              Voltage (V) | Current (AMP) | Source | ID Station | Burst interval (SECOND)|
  //        Data Interval (MINUTE)| RSSI | SNR | Time | Phone Number | Operator | Server Code | Network"));
  //TimeStamp
